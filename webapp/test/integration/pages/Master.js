		...
		onTheMasterPage : {
			...
			actions : {
				...
				iSortTheListOnName : function () {
					return this.iChooseASorter("sortButton", "Sort By <BusinessPartnerFullName>");
				},
						...
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("BusinessPartner") === oObject.BusinessPartner;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].BusinessPartner;
									} else {
										sCurrentId = aItemsNotInTheList[0].BusinessPartner;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("A_BusinessPartner", {
										BusinessPartner : sCurrentId
									});
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						...
						id: "masterPageTitle",
						autoWait: false,
						matchers: new PropertyStrictEquals({name : "text", value : "<A_BusinessPartner> (0)"}),
						success: function () {
				...
				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("BusinessPartnerFullName");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all A_BusinessPartner to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "masterPageTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "text", value : "<A_BusinessPartner> (" + iExpectedLength + ")"}),
								success : function () {
