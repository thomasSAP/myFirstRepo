		...
		onTheDetailPage : {
			...
			assertions : {
				...
				theLineItemsListShouldHaveTheCorrectNumberOfItems : function () {
					return this.waitFor(this.createAWaitForAnEntitySet({
						entitySet : "to_BusinessPartnerAddress",
						success : function (aEntitySet) {
							...
							return this.waitFor({
								...
								check: function (oList) {
									...
									var sObjectID = oList.getBindingContext().getProperty("BusinessPartner");

									var iLength = aEntitySet.filter(function (oLineItem) {
										return oLineItem.BusinessPartner === sObjectID;
									}).length;
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							return this.waitFor({
								id : "lineItemsTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name: "text", value: "<to_BusinessPartnerAddressPlural> (" + iNumberOfItems + ")"}),
								success : function () {
